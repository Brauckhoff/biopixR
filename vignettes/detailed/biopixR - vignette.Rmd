...
processing of image data is crucial to ensure we can achieve objective and reproducible 
...
of microplastics [@Ding2020], the real-time localization in microbead-based
...
Encompassing the package building, meta structure, licencing, testing,
...
## Licence and wider open source context
...
and data science. `R`'s active cummunity has developed numerous packages for a
...
designed for statistical analysis, `R`, in conjunction with its associated
...
building process and during the `R CMD check`. The unit tests corresponding to
each function are located in the `/tests/testthat/` subdirectory of the
...
threshold (`alpha`) in order to detect all objects without detecting noise, and
to maintain a minimal level of smoothing (`sigma`) in order to avoid merging
...
\phantom{x}\hspace{0.5cm} *Grid Search* - 
...
\phantom{x}\hspace{0.5cm} *Multiparameter Optimization* - 
...
```{r uncert, echo=FALSE, fig.cap="(ref:uncert)"}
include_graphics("figures/fig18_uncertainty_plot.png")
```

```{r pareto, echo=FALSE, fig.cap="(ref:pareto)"}
include_graphics("figures/fig17_Pareto_front.png")
```
...
The thresholding method is highly robust and performs also well on fluorescent
microbead images. However, this approach has one disadvantage: the threshold is
less strict than edge detection, which may result in the merging of objects in
close proximity that would be regarded as separate entities by the edge
detection method.
...
adjust these parameters in order to optimize object detection. The detected
...
are currently being detected. In order to facilitate the analysis of smaller
...
characterization of features within the `biopixR` package.
...
filter applies the 1.5 * IQR rule in order to determine the size thresholds.
...
```{r size, out.width="49%", fig.pos="ht", fig.cap="(ref:size)"}
filter_size <- sizeFilter(objects$centers, objects$coordinates, lowerlimit = 0, upperlimit = 150)

plot(objects$centers$size, ylab = "size in px")
changePixelColor(beads, filter_size$coordinates, color = factor(filter_size$coordinates$value), visualize = T)
```
...
Given that the `biopixR` package is work in progresss, there is
