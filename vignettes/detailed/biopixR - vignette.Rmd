...
```{r examples, fig.cap="(ref:examples)", out.width="99%"}
...
```{r}
include_graphics("figures/test1.png")
```
...
(ref:import2) **Example images**: Showcasing the functionality of the function for image import - `importImages()`.
...
different results (exemplary see \@ref(fig:edge1)), the Canny edge detection
...
OpenImageR::edge_detection(as.matrix(beads),
  ...
  plot(axes = FALSE,
       ...
       cex.main = 3.5)
text(c(13), c(13), c("A"), col = "darkred", cex = 4.5)

# Edge detection with 'Canny' method
cannyEdges(beads) |> plot(axes = FALSE,
                          main = "Canny - edge detection",
                          cex.main = 3.5)
text(c(13), c(13), c("B"), col = "darkred", cex = 4.5)

# Reset the plotting area to a single plot
par(mfrow = c(1, 1))
```

```{r edge1, echo = FALSE, out.width="99%", fig.cap="(ref:edge1)"}
...
package was employed to identify line ends (see \@ref(fig:edge3)B). Line ends
...
As illustrated in \@ref(fig:edge5)A, the modified Canny edge detector,
...
visual feedback as an output (\@ref(fig:edge5)B).
...
```{r object2, echo=FALSE, fig.cap="(ref:object2)", out.width="99%"}
...
backgrounds, such as transparent microbeads (see Figure \@ref(fig:object7)A).
...
```{r object7, echo=FALSE, fig.cap="(ref:object7)", out.width="99%", fig.pos="ht"}
...
`interactive_objectDetection()` function (see Figure \@ref(fig:object9)). The `alpha` and
...
threshold method for feature detection. Figure \@ref(fig:object9) illustrates that the
...
```{r object9, echo=FALSE, out.width="99%", fig.pos="ht", fig.cap="(ref:object9)"}
...
one of the images provided by the package (see Figure \@ref(fig:filter0)). The
...
```{r filter0, out.width="89%", fig.pos="ht", fig.cap="(ref:filter0)"}
...
```{r filter1, out.width="89%", fig.pos="ht", fig.cap="(ref:filter1)"}
...
included in the IQR in order to produce a representative range.

(ref:filter2) **Size Filtering Process**: The same input utilized for the previously demonstrated proximity filtering, obtained via the `objectDetection()` function, is employed in this instance. Due to an insufficient number of objects for automated calculation, the size limits are provided manually based on the size distribution shown on the left. The provided limit is visualized through a red horizontal line. On the right, the `changePixelColor()` visualization tool is employed to highlight the objects that meet the size criteria in different colors.

```{r filter2, fig.pos="ht", fig.cap="(ref:filter2)", out.width="89%"}
# Set up the plotting area to have 1 row and 2 columns
par(mfrow = c(1, 2))
...
the entire image in a single row (see Table \@ref(tab:result4)), and the second
providing detailed information about each detected object (see Table
\@ref(tab:result2)), which is useful but can be overwhelming when there are hundreds
...
\@ref(tab:imgPipe7)), with the addition of a data frame comparing the differently
encoded microbeads (see Table \@ref(tab:imgPipe6)).
...
include_graphics("figures/fig19.0_dualcolor.png")
```
...
the analysis (see Figure \@ref(fig:imgPipe7)).

(ref:imgPipe7) **Result of the Analysis of dual colored Microbeads**: The original image showing both microbead populations is presented. Detected microbeads with green emission are circled in blue, and those with red emission are circled in orange. Overlapping microbeads are not circled, due to their proximity and the resulting exclusion from the analysis.

```{r imgPipe7, fig.cap="(ref:imgPipe7)", out.width="99%"}
...
Table \@ref(tab:scan2) will be saved in the working directory path in
...
representing the results for this analysis (see Figure \@ref(fig:scan3)). The
...
(ref:scan3) **Log file created by the `scanDir()` function.** (first page)

```{r scan3, echo=FALSE, fig.cap="(ref:scan3)"}
...
channel image exclusively reveals the microbeads (see Figures \@ref(fig:fill0)A
and B).

(ref:fill0) **Images from an ePCR Assay**: **A**) Brightfield view of a water-oil emulsion, showing droplets, some of which contain microbeads. The droplet contours are highly fragmented. **B**) Fluorescent channel of the same image, displaying only the microbeads.

```{r fill0, fig.cap="(ref:fill0)", out.width="99%"}
# Set up the plotting area to have 1 row and 2 columns
par(mfrow = c(1, 2))

# Plot the droplets containing microbeads without axes
plot(droplets, axes = FALSE)
text(c(15), c(15), c("A"), col = "darkred", cex = 3.75)

# Plot the fluorescence channel (only microbeads) without axes
plot(droplet_beads, axes = FALSE)
text(c(15), c(15), c("B"), col = "darkred", cex = 3.75)
...
purple (see Figure \@ref(fig:fill1)). The method creates discrete partitions by
closing the gaps in their contours, thus facilitating the correct labeling of
the droplet partitions.

(ref:fill1) **Image of Droplets with Restored Edge Connectivity**: The result of utilizing the `fillLineGaps()` function is displayed. The pixels added by the function are highlighted in purple. It can be seen that the performance is accurate in simpler regions where straight lines are reconnected. However, in areas where smaller droplets are present between larger droplets, the reconstruction appears to be more challenging.

```{r fill1, fig.cap="(ref:fill1)", warning=FALSE, out.width="89%"}
# Restore edge connectivity with the `fillLineGaps()` function
...
The results are presented in Table \@ref(tab:fill5). The first column of the
...
text(c(20), c(20), c("A"), col = "darkred", cex = 5)
...
text(c(0.5), c(0.9), c("B"), col = "darkred", cex = 5)
```
...
include_graphics("figures/fig23_shape.png")
```
...
```{r hCfeatures, echo=FALSE, fig.cap="(ref:hCfeatures)", out.width="99%"}
#include_graphics(
#  c(
...
```{r comp, fig.cap="(ref:comp)", out.width="50%", echo=FALSE}
...
# Load the 'irr' library for calculating the Intraclass Correlation Coefficient (ICC)
library(irr)
...
extracted $\gamma$H2AX foci (see Figure \@ref(fig:DSB)). The result of the foci
quantification per cell is visualized in Figure \@ref(fig:DSB2).
...
barplot(
  event_counts,
  main = "Distribution of the number of yH2AX foci per cell",
  xlab = "foci per cell",
  ylab = "frequency",
  cex.axis = 1.2,
  cex.lab = 1.4,
...
(ref:DSB2) **Quantification of $\gamma$H2AX foci per cell**

```{r DSB2, fig.cap="(ref:DSB2)", echo=FALSE}
include_graphics("figures/fig1.1_DSB.png")
```
