...
\phantom{x}\hspace{0.5cm} *Grid Search* - In order to conduct a grid search,
...
sequence. In order to achieve a balance between the two objectives of attaining
...
measures are controlled by the input parameters `alpha` and `sigma`. In order to
...
extraction of shape features for each parameter combination in order to evaluate
them [@Binois2019].
...
In order to compare the manual and software-based methods, the Intraclass
