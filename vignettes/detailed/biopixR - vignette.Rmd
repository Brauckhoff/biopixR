...
storage, processing, and analysis [@Swedlow2009; @Peng2012]. One of the
primary objectives in this field is to achieve reproducible results, which are
...
In light of recent developments, the utilization of microscopy in the field of
biology has undergone a transformation, shifting from a predominantly visual
approach to a quantitative one [@PaulGilloteaux2023]. Bioimage informatics
plays a pivotal role in these quantitative approaches, as it enables the
efficient analysis of large volumes of image data through the application of
computational methods. One of the principal techniques employed in the
...
into distinct regions by the assignment of labels to each pixel. The primary
objective is to identify regions pertinent to the specific task [@Peng2008;
@Ghosh2019; @Niedballa2022]. Thresholding represents a relatively
straightforward segmentation method. This approach involves comparing pixel
...
The following sections explain how the `biopixR` package contributes to these
goals by providing tools for feature extraction and automation approaches for
analyzing round, spherical objects in images, such as microbeads, cells, seeds,
or microplastics, which have quite similar characteristics in regards to their
...
Our primary objective was to publish the package on the Comprehensive R Archive
Network (CRAN), which mandates high-quality software. This objective has been
...
The aim of the `biopixR` package is to provide the necessary features for
comprehensive image processing in the field of life science, including:
