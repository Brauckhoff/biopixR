...
capabilities in the analysis and processing of biological images (see Figure
\@ref(fig:examples2)). The sample images illustrate the package's functionalities,
...
cell-based assays. Therefore this vignette will primarily focus on the analysis
of microbead particles made of polymethylmethacrylate (PMMA), which are
approximately 12 µm in size [@Geithe2024].
...
`importImage()` function converts all formats into the `imager` class 'cimg'.
During the development process, it was frequently observed that images exhibited
more than three dimensions within the color channel, specifically an additional
transparency layer, also known as alpha. Such images often lead to challenging
and elusive errors. To address this issue, the `imageImport()` function employs
a process of detection and removal of the fourth color dimension, if present.
The function supports the importation of digital images in various file formats,
...
Various edge detection algorithms are available in the `R` programming language,
...
@Scharr2000. After evaluating the different results (exemplary see Figure
...
exhibited gaps, rendering them inaccessible for subsequent labeling (see Figure
...
package was employed to identify line ends (see Figure \@ref(fig:edge3)B). Line
...
visual feedback as an output (see Figure \@ref(fig:edge5)B).
...
nearby objects as seen in normal thresholding (see Figure \@ref(fig:object7)). In
...
space is called the Pareto front [@Binois2019] (see Figure \@ref(fig:object4)).
...
The principle of this method involves: \linebreak
1. Generating an initial set of observations. \linebreak
2. Fitting the Gaussian process models to each objective independently. \linebreak
3. Running an inner optimization loop to find the best point (new point as the maximizer of an infill criterion). \linebreak
...
`objectDetection()` function. The microbeads are displayed in Figure
\@ref(fig:object5), with all microbeads successfully identified.
...
against inconsistent backgrounds, such as transparent microbeads (see Figure
\@ref(fig:object7)A). The thresholding method does not require any additional
...
`interactive_objectDetection()` function (see Figure \@ref(fig:object9)). The
...
## Dealing with induced Fluorescence and Aggregation with `sizeFilter()` and `proximityFilter()`
...
has the potential to induce fluorescence in adjacent microbeads and result in the
...
the image as input and calculates these properties. The results are presented in
...
(see Table \@ref(tab:result2)), and the second providing detailed information
about each detected object (see Table \@ref(tab:result4)), which is useful but
...
footnote(summary, general = "sd: standard deviation",
...
footnote(detailed, general = "px: pixel, sd: standard deviation",
         general_title = "Abbreviations: ")
```
...
similar format as presented in Chapter \@ref(result) (see Table \@ref(tab:imgPipe5)), 
with the addition of a data frame comparing the differently
...
```{r, out.width="99%", echo=TRUE}
include_graphics("figures/fig19.1_dualcolor.png")
```
...
footnote(extr_features, general = "sd: standard deviation",
         general_title = "Abbreviations: ")
```
...
footnote(dual_col, general = "sd: standard deviation",
         general_title = "Abbreviations: ")
```
...
```{r imgPipe7, fig.cap="(ref:imgPipe7)", out.width="85%"}
...
\phantom{x}\hspace{0.5cm} *MD5 sum* - To guarantee the quality of the
analyses, the function incorporates a verification process to ensure that the
...
algorithm [@Rivest1992] is a standard practice in computer science for
...
prompted to remove the duplicated files.
...
`biopixR` package (see Figure \@ref(fig:examples2)). The 'threshold' method is
...
footnote(tab_scan, general = "sd: standard deviation",
         general_title = "Abbreviations: ")
```
...
comparability. Moreover, the detailed results (see Table \@ref(tab:result4)) for
...
applied to segment the contours and microbeads. Subsequently, object detection
...
```{r}
include_graphics("figures/fig25_fill.png")
```
...
microbeads within a single droplet. Figure \@ref(fig:fill6) provides a visual
representation of the result. The different microbeads containing droplets are
...
area, perimeter, radius, eccentricity, circularity, and aspect ratio (AR).
...
individual shape features of each microbead in the image are displayed as
boxplots (see Figure \@ref(fig:shape2)B). The characteristics of the microbeads
marked with a red point in Figure \@ref(fig:shape2)A are also highlighted with a
red point within the boxplot. The distinction between doublets and multiplets is
readily apparent. Doublets and multiplets are characterized by their larger
size, perimeter, and radius, as well as lower circularity and higher
eccentricity. This reflects their oval appearance, which differentiates them
from single microbeads.
...
text(c(0.5), c(0.9), c("B"), col = "darkred", cex = 4)
```
...
include_graphics("figures/fig23.1_shape.png")
```

To validate the AR calculations performed using the `biopixR`
package, a reference figure containing circles with known ground truth aspect
ratios was employed for comparison (see Figure \@ref(fig:shape3)). The aspect
ratio represents the ratio of an object's dimensions and can be described as
follows [@Takashimizu2016]:
...
(ref:shape3) **Circles with different Aspect Ratios** (ARs): This is a modified image, originally derived from the publication of @Takashimizu2016. The enumeration of objects is read from left to right, beginning with the object in the top left (see Table \@ref(tab:shape7)).

```{r shape3, echo=FALSE, out.width="69%", fig.cap="(ref:shape3)"}
include_graphics("figures/fig2_circularity.png")
```

The results of the analysis of Figure \@ref(fig:3) using the `shapeFeatures()`
function are presented in Table \@ref(tab:shape7), which demonstrates the ability
...
truth data (see Figure \@ref(fig:shape8)). Notably, the calculations for the
...
footnote(tab_AR, general = "sd: standard deviation",
         general_title = "Abbreviations: ")
```

(ref:shape8) **Performance of the `shapeFeatures()` function in regards to the Aspect Ratio**: This plot compares the aspect ratio calculated by the `biopixR` package with ground truth data from a publication [@Takashimizu2016]. The ground truth data is shown in blue, and the obtained results are shown in green.

```{r shape8, echo=FALSE, fig.cap="(ref:shape8)"}
...
by the function is presented in Table \@ref(tab:shape7). In the initial example,
...
function to four images of microbeads, provided by the `biopixR` package (see
Figure \@ref(fig:example2)). The resulting output is presented in Table
\@ref(tab:haralickCluster1). The results of the Haralick texture feature
