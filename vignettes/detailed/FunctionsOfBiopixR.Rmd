...
\@ref(fig:examples2)). For more details about microbead assays and their 
application in biomedical research, please see this reference [@Roediger2014; @].
...
﻿Edge detection (e.g., contours, lines) is crucialt in various applications 
such as computer vision and object recognition, particularly in 
medical imaging analysis. It aids in identifying regions with significant 
...
@Scharr2000. After evaluating the different results (e.g., see Figure
...
nearby objects as seen in thresholding (see Figure \@ref(fig:object7)). In
...
radius (see Figure \@ref(fig:filter1)). For each object, a square region is
...
(see Table \@ref(tab:result3)), and the second providing detailed information
about each detected object (see Table \@ref(tab:result6)), which is useful but
...
frame comparing the differently encoded microbeads (see Table \@ref(tab:imgPipe6)
...
the analysis (see Figure \@ref(fig:imgPipe9)).
...
\phantom{x}\hspace{0.5cm} *MD5 sum* - To elevate the quality of the
...
since it may lead to false positives or negatives. However, we use it in 
combination with the file name, which leverages the problem. In theory, this 
could pose issues in scenarios where hash values are employed for security 
purposes or data integrity checks since it may result in false positives or 
negatives. However, we can additionally employ the file name and meta data (e.g., 
file creation date) alongside these hash values to counteract this issue.
...
The following page (Figure \@ref(fig:scan4)) present the log file generated from the `RMarkdown` file
representing the results for this analysis. The
...
Furthermore, the log file will indicate any instances of analysis failure resulting
...
overview for verification of the analysis's compliance with
...
microbeads exist within a watery reaction environment that is surrounded by 
...
channel image exclusively reveals the microbeads (see Figures \@ref(fig:fill1)A
and B).
...
internal visualization that highlights the added pixels in purple (see Figure
...
patterns in shape-related characteristics of image objects.
...
comparison (see Figure \@ref(fig:shape3)). The AR represents the ratio of an
...
(ref:shape3) **Circles with different Aspect Ratios** (ARs): This is a modified image, originally derived from the publication of @Takashimizu2016. The enumeration of objects is read from left to right, beginning with the object in the top left (see Table \@ref(tab:shape8)).
...
truth data (see Figure \@ref(fig:shape9)). Notably, the calculations for the
...
structures within an image that can provide important information like intensity 
...
Figure \@ref(fig:examples2)). The resulting output is presented in Table
