...
```{r cyclocomp2}
row_spec(tab, c(1, 3:14, 16, 20), background = "yellow")
```
...
\@ref(fig:irr0)) (n=5) with the results obtained using the `biopixR` package.
...
which are displayed in Figure \@ref(fig:irr0). The images were selected for
analysis because they depict varying amounts of microbeads and the phenomena of
clotted microbeads.

(ref:irr0) **Microbead Images for Manual Analysis**: These three images represent varying quantities of microbeads. Each image displays a unique combination of dimensions and the presence of doublets and multiplets at varying amounts.
...
for analysis is depicted in Tables \@ref(tab:irr2) and \@ref(tab:irr3).
...
presented in Table \@ref(tab:irr2). It is evident that manual counting for the
...
\@ref(tab:irr3). The results from both the software and manual analysis of
...
in Figure \@ref(fig:irr5), which demonstrates that the software and manual
...
extracted $\gamma$H2AX foci (see Figure \@ref(fig:DSB1)). The result of the foci
quantification per cell is visualized in Figure \@ref(fig:DSB3).
...
(ref:DSB3) **Quantification of $\gamma$H2AX foci per cell**

```{r DSB3, fig.cap="(ref:DSB3)", echo=FALSE}
