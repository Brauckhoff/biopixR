...
with(
  ...
  points(res_objectDetection$centers$mx,
...
use the `sizeFilter` to achieve this goal.
...
```{r}
DT <- data.table(res_sizeFilter$coordinates)
res_center <- DT[, .(x = mean(x), y = mean(y)), by = value]
...
plot(beads)
with(
  res_proximityFilter$centers,
  points(res_proximityFilter$centers$mx,
...
`proximityFilter`: The function works by examining
each collected center and scanning a specific radius for positive pixels. If
another positive pixel of a different microbead is detected within this range, 
both are discarded due to their proximity. 

This method is especially efficient in dealing with multiplets as the
...
```{r}
result <- resultAnalytics(coordinates = res_proximityFilter$coordinates, 
...
```{r}
ind_sizeFilter <- sizeFilter(centers = res_objectDetection$centers,
                             coordinates = res_objectDetection$coordinates,
                             lowerlimit = 50,
                             upperlimit = 150)

result_sizeFilter <- resultAnalytics(coordinates = ind_sizeFilter$coordinates,
...
```{r}
ind_proximityFilter <- proximityFilter(centers = res_objectDetection$centers,
                                       coordinates = res_objectDetection$coordinates,
                                       radius = 'auto')

result_proximityFilter <- resultAnalytics(coordinates = ind_proximityFilter$coordinates,
...
```{r}
DT <- data.table(res_proximityFilter$coordinates)
res_center <- DT[, .(x = mean(x), y = mean(y)), by = value]
```
...
*will be part of the `resultAnalytics` function as of version 0.1.1*
