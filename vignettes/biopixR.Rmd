...
sample images displays the package's features, enabling users to experiment with
...
another function, which will be introduced later. This fitness function
evaluates the results with different input parameters, assuming circular-shaped
objects. While the grid search method can be time-consuming as it tests every
possible combination, the Pareto front optimization method samples and analyzes
a subset of combinations, estimating the optimal parameters more quickly.
...
the decision between greater accuracy with parameter input or calculation and
the more straightforward thresholding approach depends on the user's specific
requirements.
...
distribution plot. Currently, this interactive module is only triggered when the
number of detected objects is less than 50. In future updates (expected in
v1.2.0), it will also be possible to invoke this module by setting `lowerlimit`
and `upperlimit` to 'interactive'.
...
## Relative distance between microparticles !!!to be removed!!!
...
```{r}
dist_mat <- dist(res_proximityFilter$centers[, 2:3], method = 'euclidean')
...
```{r}
summary_stats_fun <- function(distances) {
  # Exclude the diagonal element (distance to itself)
  non_zero_distances <- distances[distances != 0]
  c(
    mean = mean(non_zero_distances),
    median = median(non_zero_distances),
    max = max(non_zero_distances),
    min = min(non_zero_distances)
  )
}

# Apply the custom function to each row of the distance matrix
summary_stats <- t(apply(dist_mat, 1, summary_stats_fun))
...
result$detailed$relative_distance <- summary_stats_df$mean
result$detailed
```
...
The relative distance metric was initially removed from the function and
replaced by the coverage metric due to performance concerns and lengthy
computation times. However, it will be reintroduced with the presented approach
in the upcoming v1.2.0 release.

# Case study I: Using the `biopixR` image analysis pipeline
...
# Case study II: Clustering of image objects and images

haralickCluster


```{r haralickCluster1, fig.width = 10, fig.height = 10}
par(mfrow = c(2, 2))

plot(beads, main = "Beads")
plot(beads_large1, main = "Beads_large1")
plot(beads_large2, main = "Beads_large2")
plot(droplet_beads, main = "Droplet_beads")
...
```{r shapeFeatures2}
res_shapeFeatures <- shapeFeatures(
  beads_large1,
  alpha = 0.8,
  sigma = 0.7,
  SOM = TRUE
...
colors <- c("cyan", "red")
plot(beads_large1)
points(res_shapeFeatures$x, res_shapeFeatures$y, col = colors[factor(res_shapeFeatures$class)], pch = 20)
```


# Case study III: microbeads in droplets
