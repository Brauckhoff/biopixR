...
crucial to exclude multiplets and doublets from subsequent analysis. The
captured contours of multiplets result in a recorded size considerably smaller
than that of a singlet. Conversely, fully recorded doublets are approximately
twice as large as singlets. Therefore, the selected parameter to address these
microbeads will be their size. The following section will provide a detailed
explanation of the filter application procedure.
...
color, simplifying the adjustment of the threshold and smoothing parameter in
the `objectDetection` function.
...
## Filter microbeads according to size and proximity {#filter}
...
are identified, automated limit calculation becomes available based on the mean
and standard deviation of the size distribution. To simplify the selection of
limits in cases where the number of detected objects is insufficient, the
function will terminate and produce a size distribution.
...
As shown by the size distribution, there is a larger object (doublet - size:
\>150) and several smaller objects representing the edges of the multiplet
(multiplet edge - size: \<50). Therefore, the limits will be set accordingly.
...
a bead between them were removed with the aid of the `proximityFilter`. To
...
the percentage of positive pixels (microbeads) relative to the pixel area of the
...
observer may have noticed that the actual count of discarded microbeads differs
...
accurate result, is not feasible. The initial approach was to use an automatic
threshold to identify all coordinates marked as TRUE, followed by a count of
those coordinates. This count was then divided by the average size of the
microbeads, and after subtracting the number of passing microbeads, the
resulting value represented the number of discarded objects. A potential
limitation arose from the occurrence of overlapping microbeads, as their common
area was only counted once in the image, resulting in an underestimated number
of discarded microbeads. In addition, the method relied heavily on accurate size
information, raising concerns about its robustness in cases of varying object
sizes.

To address these challenges and provide a more reliable estimate, a simplified
approach was adopted. The estimated number of rejected objects was calculated by
examining the number of detected centers and the number of centers that
successfully passed the filtering procedure. However, this approach also
inevitably introduced errors associated with multiplets where numerous centers
were detected, leading to inaccuracies in the final estimate. However, in this
scenario, rejected objects are overestimated rather than underestimated.
Additionally, non-uniform object sizes have resulted in negative values when
using the aforementioned approach. Therefore, since multiplets are expected to
be infrequent and significantly outnumbered by singlets, we have chosen an
approach that focuses on analyzing individual centers.
...
This filter is effective in addressing multiplets as the identified centers
often appear in close proximity along the contours. It is important to note that
only the multiplet has been excluded, which is unexpected given the earlier
removal of microbeads 1, 2, 29, and 30. An explanation for this inconsistency is
necessary. The absence of the previous sizeFilter allows the multiplet edge with
the small object size to persist in the data set, substantially influencing the
determination of the radius. The average size is lowered by the presence of
small objects, which in turn limits the examined surrounding area due to a
reduction in the assumed radius. If the function were to be repeated, the
original result at the end of Chapter ['Filter microbeads'](#filter) would be
replicated, as the size would align with the true microbead size after the
exclusion of the multiplet.
...
The results of the `objectDetection` can be displayed using the
`resultAnalytics` function. However, for this sample image, this is not useful,
so we will move on to the next topic.
