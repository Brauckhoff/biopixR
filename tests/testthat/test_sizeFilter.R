...
test_that("sizeFilter", {
  ...
  expect_equal(
    class(res_sizeFilter$remaining.coordinates.s),
    c("data.frame")
  )
  expect_lte(
  ...
  expect_equal(
    length(which(res_sizeFilter$remaining.coordinates.s$cluster == 1)),
    unlist(res_sizeFilter$size[1])
  )
  expect_equal(
    max(res_sizeFilter$remaining.coordinates.s$cluster),
    length(res_sizeFilter$size)
  )
